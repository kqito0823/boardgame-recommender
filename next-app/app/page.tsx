...
export default function Home() {
    ...
    // ジャンル一覧
    const genres = [
        { id: "bluff", label: "騙しあい系（ブラフ系）" },
        { id: "concealing", label: "正体隠し系（正体隠匿）" },
        { id: "tile", label: "タイル配置系" },
        { id: "grow", label: "発展系（拡大再生産系）" },
        { id: "draft", label: "選び取り系（ドラフト系）" },
        { id: "worker-placement", label: "コマ配置系（ワーカープレースメント）" },
        { id: "territory", label: "陣取り系" },
        { id: "deck", label: "デッキ構築系" },
        { id: "auction", label: "オークション系" },
        { id: "coop", label: "協力系" },
        { id: "expression", label: "表現系" },
        { id: "action", label: "アクション系" },
    ];
    ...
    const onSubmit: SubmitHandler<Inputs> = async (data) => {
        ...
        try {
            ...
            if (!reader) throw new Error(`ストリームの読み込みに失敗しました: ${response.status}`);
        ...
        <div className="flex flex-col items-center justify-center max-w-6xl px-20">
            ...
            <div className="w-full p-6 mt-10 bg-white border rounded-lg shadow-sm min-h-50">
                ...
                <div className="mt-20 text-gray-800 whitespace-pre-wrap">
                    {content || (
                        <span className="text-gray-400">ここに生成結果が表示されます...</span>
                    )}
                </div>
            </div>
        </div>
